...
def _row_to_dict(row: tuple) -> dict:
    return {
        ...
        "can_manage_links": bool(row[6]) if len(row) > 6 else False,
...
def create_user(body: CreateUser, _=Depends(require_admin)):
    ...
    manager.create_panel_user(discord_id, body.discord_name, body.is_admin, body.can_view_logs, body.can_control_bots, body.can_fetch_api, body.can_manage_links)
...
def update_user(discord_id: int, body: UpdateUser, request: Request, claims=Depends(require_admin)):
    ...
    manager.update_panel_user_permissions(discord_id, final_is_admin, body.can_view_logs, body.can_control_bots, body.can_fetch_api, body.can_manage_links)
