...
def create_ipc_app(client):
    ...
    async def get_guild_members(key: str):
        ...
        if not state.connected or not state.bot:
            ...
            members = [{"ign": r[0], "rank": r[1], "skyblock_level": r[2], "last_login": r[3], "uuid": r[4] or None, "discord_name": r[5] or None, "discord_id": str(r[6]) if r[6] else None, "online": False} for r in rows]
        ...
        rows = manager.get_guild_members(key)
        members = [{"ign": r[0], "rank": r[1], "skyblock_level": r[2], "last_login": r[3], "uuid": r[4] or None, "discord_name": r[5] or None, "discord_id": str(r[6]) if r[6] else None, "online": r[0] in online_igns} for r in rows]
